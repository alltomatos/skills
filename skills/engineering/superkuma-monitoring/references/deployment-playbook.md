...
site's own infrastructure, typically a Proxmox VE host reached over VPN. Covers the concrete
pitfalls hit deploying to real Proxmox clusters in the field; read this before the
...
Reuse the same shape as every other instance this skill has deployed: SuperKuma + MariaDB (per the
SuperKuma repo's ADR-0009, "master: long-term metrics history" — MariaDB is the
...
Verified working in the field: the bundled node-exporter reported this VM's real
`node_memory_MemTotal_bytes`/CPU immediately with no extra config, and a remote Proxmox-host
node_exporter (installed per prometheus-exporters.md) was reachable and scraped successfully over
the LAN with `job: health: up`.
...
Rolling out a new SuperKuma version to an already-deployed instance (not a fresh install — see
sections 1-2 above for that).

### 5.1 Get a new image

**If you just consume the public image** (the common case): check
[hub.docker.com/r/ronaldodavi/superkuma/tags](https://hub.docker.com/r/ronaldodavi/superkuma/tags)
(or `docker pull ronaldodavi/superkuma:latest` and read the version it reports) for the newest tag
and skip to §5.2 — nothing to build.

**If you maintain your own fork/build of SuperKuma from source** (self-hosting a customized
version), you own the release pipeline end to end:
...
1. Bump the version in `package.json` (the `version` field **and** the `scripts.setup`
   ...
   that happen to match the same number by coincidence). Commit and push/merge to your default
   branch.
...
2. Tag the commit (`git tag vX.Y.Z && git push --tags`) and build the image yourself
   (`docker build -t <your-registry>/superkuma:X.Y.Z .`), or wire this into your own CI. If you
   automate tagging via a CI workflow whose push uses the workflow's own token, remember GitHub's
   anti-recursion rule: that push won't cascade into another tag-triggered workflow — dispatch the
   build step explicitly (`gh workflow run <build-workflow> --ref main -f version=X.Y.Z`) rather
   than assuming it fires on its own.
3. Push the built image to whatever registry your compose file references (Docker Hub, GHCR, a
   private registry, …), tagged at minimum `X.Y.Z`; a `latest`/major-version alias is convenient
   but optional.
