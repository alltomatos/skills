...
for the **healthy** side, `<=` `90`, not `>` `90` — verified the hard way in the field: monitors
created with the intuitive-but-inverted operator all reported DOWN despite healthy hosts, e.g.
`19% free RAM < 10` evaluating false → DOWN, when 19% free is fine. Always
